...
public final class DateHelper {
    ...
    public static Date parseDateTime(String dateString) {
        try {
            return new SimpleDateFormat(DcsFormats.DATE_TIME_FORMAT).parse(dateString);
        } catch (final ParseException e) {
            throw new IllegalArgumentException(e);
        } catch (final RuntimeException e) {
            throw new RuntimeException("Error while parsing date string '" + dateString
                            + "' with format pattern "
                            + DcsFormats.DATE_TIME_FORMAT, e);
        }
    }

    public static String formatToDate(Date date) {
        return new SimpleDateFormat(DcsFormats.DATE_FORMAT).format(date);
    }

    public static String formatToHour(Date date) {
        return new SimpleDateFormat(DcsFormats.HOUR_FORMAT).format(date);
    }

    public static String formatToDateTime(Date date) {
        return new SimpleDateFormat(DcsFormats.DATE_TIME_FORMAT).format(date);
    }
    ...
    public static Date parseToValueDate(String date) {
        Date result = null;
        try {
            result = new SimpleDateFormat(DcsFormats.DATE_FORMAT).parse(date);
    ...
    public static Date parseIso8601(String isoDate) {
        try {
            return new SimpleDateFormat(DcsFormats.ISO8601_FORMAT).parse(isoDate);
        } catch (final ParseException e) {
    ...
    public static String formatIso8601(Date isoDate) {
        return new SimpleDateFormat(DcsFormats.ISO8601_FORMAT).format(isoDate);
    }
