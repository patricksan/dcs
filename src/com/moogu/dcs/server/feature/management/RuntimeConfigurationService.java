...
//TODO "key" is a ansi sql reserved word. Shouldn't be refactored to a different name?
@Component
public class RuntimeConfigurationService {
    ...
    public static String[] KEYS = { DcsConfigConstants.RESERVE_EUR_OVERVIEW_SCREEN_REFRESH_SECONDS };
    ...
    public String getProperty(String key) {
        final List<Map<String, Object>> list = jdbcTemplate.queryForList(
                        "select value from TB_PROPERTIES prop where prop.key = ?",
                        key);
        if (list.isEmpty()) {
	...
	public void setProperty(String key, String value) {
        final int keyCount = jdbcTemplate.queryForInt("select count(*) from TB_PROPERTIES prop where prop.key = ?", key);
        if (keyCount > 0) {
            jdbcTemplate.update("update TB_PROPERTIES prop set prop.value = ? where prop.key = ?", value, key);
        } else {
